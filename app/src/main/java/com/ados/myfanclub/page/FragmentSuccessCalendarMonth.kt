...
class FragmentSuccessCalendarMonth : Fragment() {
    ...
    private fun initView(view: View) {
        ...
        val fieldName = SimpleDateFormat("yyyy").format(date)
        ...
        if (param1.equals("fanClub")) {
            ...
            firebaseViewModel.getFanClubScheduleStatistics(fanClubDTO?.docName.toString(), currentMember?.userUid.toString(), "month", fieldName)
        } else if (param1.equals("personal")) {
            val user = (activity as MainActivity?)?.getUser()
            firebaseViewModel.getPersonalScheduleStatistics(user?.uid.toString(), "month", fieldName)
        }

        firebaseViewModel.scheduleStatistics.observe(viewLifecycleOwner) {
            recyclerViewAdapter = RecyclerViewAdapterSuccessCalendarMonth(date, firebaseViewModel.scheduleStatistics.value!!, successCalendar)
            ...
            if (binding.progressPercent.progress < 100) {
                ...
                when {
                    binding.progressPercent.progress < 40 -> {
                        binding.progressPercent.progressBackgroundTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_background_0))
                        binding.progressPercent.progressTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_0))
                    ...
                    binding.progressPercent.progress < 70 -> {
                        binding.progressPercent.progressBackgroundTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_background_40))
                        binding.progressPercent.progressTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_40))
                        binding.textPercent.setTextColor(ContextCompat.getColor(requireContext(), R.color.progress_40))
                    }
                    else -> {
                        binding.progressPercent.progressBackgroundTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_background_70))
                        binding.progressPercent.progressTintList = ColorStateList.valueOf(
                            ContextCompat.getColor(requireContext(), R.color.progress_70))
                ...
                binding.progressPercent.progressBackgroundTintList = ColorStateList.valueOf(
                    ContextCompat.getColor(requireContext(), R.color.progress_background_100))
